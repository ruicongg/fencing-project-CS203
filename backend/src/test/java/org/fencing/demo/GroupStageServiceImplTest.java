...
class GroupStageServiceImplTest {
    ...
    @Test
    public void getAllGroupStagesByEventId_ValidId_ReturnsListOfGroupStages() {
        Long eventId = 1L;
        Long groupStage1Id = 1L;
        Long groupStage2Id = 2L;

        Event event = new Event();
        event.setId(eventId);

        GroupStage groupStage1 = new GroupStage();
        groupStage1.setId(groupStage1Id);
        GroupStage groupStage2 = new GroupStage();
        groupStage2.setId(groupStage2Id);
        List<GroupStage> groupStages = List.of(groupStage1, groupStage2);

        when(eventRepository.findById(eventId)).thenReturn(Optional.of(event));
        when(groupStageRepository.findAllByEventId(eventId)).thenReturn(groupStages);

        List<GroupStage> result = groupStageService.getAllGroupStagesByEventId(eventId);

        assertNotNull(result);
        assertEquals(2, result.size());
        assertEquals(groupStages, result);

        verify(eventRepository, times(1)).findById(eventId);
        verify(groupStageRepository, times(1)).findAllByEventId(eventId);
    }

    @Test
    public void getAllGroupStagesByEventId_NullId_ThrowsIllegalArgumentException() {
        assertThrows(IllegalArgumentException.class, () -> {
            groupStageService.getAllGroupStagesByEventId(null);
        });
    }

    @Test
    public void getAllGroupStagesByEventId_NonExistentId_ThrowsEventNotFoundException() {
        Long eventId = 1L;

        when(eventRepository.findById(eventId)).thenReturn(Optional.empty());

        assertThrows(EventNotFoundException.class, () -> {
            groupStageService.getAllGroupStagesByEventId(eventId);
        });

        verify(eventRepository, times(1)).findById(eventId);
        verify(groupStageRepository, never()).findAllByEventId(eventId);
    }
