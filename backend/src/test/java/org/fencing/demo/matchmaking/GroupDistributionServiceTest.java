...
class GroupDistributionServiceTest {
    ...
    void testDistributePlayersIntoGroups_success() {
        ...
        assertEquals(1000, group0.get(0).getElo());
        // everything in between
        assertEquals(1100, group1.get(0).getElo());
        assertEquals(1200, group0.get(1).getElo());
        assertEquals(1300, group1.get(1).getElo());
        assertEquals(1400, group0.get(2).getElo());
        assertEquals(1500, group1.get(2).getElo());
        assertEquals(1600, group0.get(3).getElo());

        // highest ELO player is in group 1
        assertEquals(1700, group1.get(3).getElo());
    }
    ...
    void distributePlayersIntoGroups_NullInput_ReturnsNull() {
        Map<Integer, List<Player>> result = groupDistributionService.distributePlayersIntoGroups(null);
    ...
    void distributePlayersIntoGroups_EmptySet_ReturnsNull() {
        ...
        Map<Integer, List<Player>> result = groupDistributionService.distributePlayersIntoGroups(emptySet);
    ...
    private void verifyGroupDistribution(Map<Integer, List<Player>> groups) {
        List<Player> group0 = groups.get(0);
        List<Player> group1 = groups.get(1);
        
        assertEquals(1200, group0.get(0).getElo());
        assertEquals(1300, group1.get(0).getElo());
        assertEquals(1250, group0.get(1).getElo());
        assertEquals(1350, group1.get(1).getElo());
        assertEquals(1400, group0.get(2).getElo());
        assertEquals(1500, group1.get(2).getElo());
        assertEquals(1600, group0.get(3).getElo());
        assertEquals(1800, group1.get(3).getElo());
    }
